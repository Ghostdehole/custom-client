...
def generator_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            addcopykey = form.cleaned_data['addcopykey']
            ...
            extras['hideQuit'] = 'true' if hideQuit else 'false'
            extras['addcopykey'] = 'true' if addcopykey else 'false'
...
def maintenance_view(request):
    status = getattr(request, '_error_status', 503)
    return render(request, 'maintenance.html', status=status)
